(function(){
 ...
 function playerOptionsForTeam(state,teamId,search='',selected='',kind='goal'){
   ...
   if(q){
     ...
     // L'autogol resta cercabile con “autogol”, “auto” o “own goal”.
     if(/^\d+$/.test(q)){
       filtered=people.filter(p=>p.type!=='own-goal'&&String(p.number??'').startsWith(q));
   ...
   return '<option value="">Seleziona marcatore/autogol</option>'+filtered.map(p=>`<option value="${UI.esc(p.id)}" ${p.id===selected?'selected':''}>${UI.esc(p.label)}</option>`).join('')+(filtered.length?'':'<option value="" disabled>Nessun risultato</option>');
 }
 ...
 function draftFromMatch(m){return {field:m.field||'',referee:m.referee||'',date:m.date||'',time:m.time||'',status:m.status||'scheduled',penaltiesHome:m.penalties?String(m.penalties.home):'',penaltiesAway:m.penalties?String(m.penalties.away):'',goals:(m.goals||[]).map(g=>store.isOwnGoalEvent&&store.isOwnGoalEvent(g)?{ownGoal:true,teamId:g.teamId,weight:1}:{playerId:g.playerId,weight:Number(g.weight)===2?2:1}),cards:(m.cards||[]).map(c=>({playerId:c.playerId,type:c.type==='red'?'red':'yellow'}))};}
 ...
 function syncFormDraft(form){
   ...
   const nextGoals=[];
   const rows=Math.max(goalPlayerIds.length,goalOwnFlags.length,goalTeamIds.length,goalWeights.length);
   for(let i=0;i<rows;i++){
     const own=String(goalOwnFlags[i]||'')==='1';
     if(own){
       const teamId=String(goalTeamIds[i]||'');
       if(m&&(teamId===m.homeTeamId||teamId===m.awayTeamId)) nextGoals.push({ownGoal:true,teamId,weight:1});
       continue;
     }
     const playerId=String(goalPlayerIds[i]||'');
     if(!playerId)continue;
     if(store.isPresidentId(state,playerId)&&!isPresidentScorerAllowed(state))continue;
     nextGoals.push({playerId,weight:isKings(state)&&Number(goalWeights[i])===2&&!store.isPresidentId(state,playerId)?2:1});
   }
   ...
   reportDrafts.set(matchId,{
     ...
     cards:(cardIds.length||hasCardEditor)?cardIds.map((playerId,i)=>({playerId,type:cardTypes[i]==='red'?'red':'yellow'})):current.cards||[]
   });
 }
 ...
 function draftGoalRows(s,draft){return (draft.goals||[]).filter(g=>g.ownGoal||(g.playerId&&(!store.isPresidentId(s,g.playerId)||isPresidentScorerAllowed(s)))).map(g=>goalDraftItem(s,g.ownGoal?g:g.playerId,isKings(s)?(g.weight||1):1)).join('');}
 ...
 function draftScoreFromDraft(state,m,d){
   ...
   const draftMatch={...m,goals:(d.goals||[]).map(g=>g.ownGoal?{ownGoal:true,teamId:g.teamId,playerId:'',weight:1}:{playerId:g.playerId,weight:g.weight||1})};
 ...
 function matchCommandHtml(s,m){
   const d=getReportDraft(m);
   const home=store.teamName(s,m.homeTeamId,m.homeLabel), away=store.teamName(s,m.awayTeamId,m.awayLabel);
   const draftMatch={...m,field:d.field,referee:d.referee,date:d.date,time:d.time,status:d.status||m.status,goals:(d.goals||[]).map(g=>g.ownGoal?{ownGoal:true,teamId:g.teamId,playerId:'',weight:1}:{playerId:g.playerId,weight:g.weight||1}),cards:(d.cards||[])};
...
Verrà svuotato il campo arbitro in ${withRef.length} partita/e. Campo, data, orario e referti non verranno modificati.`);
 ...
 function persistMatchContext(s0, draft, finalStatus){
   ...
   const safeGoals=(draft.goals||[]).filter(g=>{
     if(g.ownGoal)return m0&&(g.teamId===m0.homeTeamId||g.teamId===m0.awayTeamId);
     return g.playerId&&(!store.isPresidentId(s0,g.playerId)||isPresidentScorerAllowed(s0));
   });
   const safeCards=(draft.cards||[]).filter(c=>c.playerId&&!store.isPresidentId(s0,c.playerId));
   const draftMatch={...m0,goals:safeGoals.map(g=>g.ownGoal?{ownGoal:true,teamId:g.teamId,playerId:'',weight:1}:{playerId:g.playerId,weight:g.weight||1})};
   ...
   A.commit(s=>{
     ...
     m.goals=safeGoals.map(g=>g.ownGoal?{id:store.uid('goal'),ownGoal:true,teamId:g.teamId,playerId:'',weight:1}:{id:store.uid('goal'),playerId:g.playerId,weight:isKings(s)&&Number(g.weight)===2&&!store.isPresidentId(s,g.playerId)?2:1});
