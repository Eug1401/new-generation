(function(){
  ...
  function normalizeGoalEvent(g,m=null){
    ...
    if(isOwnGoalEvent(g)){
      ...
      return {id:base.id,playerId:'',ownGoal:true,teamId:valid?teamId:'',weight:1};
    }
    return base;
  }
  ...
  function deriveFingerprint(state){
    ...
    const matchesKey=(state.matches||[]).map(m=>({
      ...
      goals:(m.goals||[]).map(g=>({id:g.id,playerId:g.playerId,ownGoal:!!g.ownGoal,teamId:g.teamId,weight:Number(g.weight)||1})),
      cards:(m.cards||[]).map(c=>({id:c.id,playerId:c.playerId,type:c.type}))
    }));
